...
public class grille {
    ...
    public String placerNavire (personnage p1){
		...
		for (int i=0; i<ligne; i++) {	
			for (int j=0; j<colonne; j++) {
				//Si différent d'une case vide ET balayage dans toute les position différent de la taille, on relance le random
				if (grille [ligneG][colonneG] != "--" && (balayageCollision("haut", p1, ligneG, colonneG) != p1.taille || balayageCollision("bas", p1, ligneG, colonneG) != p1.taille ||
				balayageCollision("gauche", p1, ligneG, colonneG) != p1.taille || balayageCollision("droite", p1, ligneG, colonneG) != p1.taille)){
